...
namespace Graphics
{
    ...
    BoolVar g_bVRS1Supported( "Graphics/AllowVRS", false, false );
    bool g_bVRS1AdditionalRatesSupported = false;
...
void Graphics::Initialize(void)
{
    ...
    if ( SUCCEEDED( g_Device->CheckFeatureSupport( D3D12_FEATURE_D3D12_OPTIONS6, &FeatureData6, sizeof( FeatureData6 ) ) ) )
    {
        if ( FeatureData6.VariableShadingRateTier >= D3D12_VARIABLE_SHADING_RATE_TIER_1 )
        {
            g_bVRS1Supported.SetCanChange( true );
            g_bVRS1AdditionalRatesSupported &= TRUE == FeatureData6.AdditionalShadingRatesSupported ? true : false;
        }
    }
    ...
    ASSERT_SUCCEEDED(dxgiFactory->CreateSwapChainForHwnd(g_CommandManager.GetCommandQueue(), GameCore::g_hWnd, &swapChainDesc, &fullscreenDesc, nullptr, &s_SwapChain1));
#else // UWP
