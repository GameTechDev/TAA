...
{
    if ( nullptr != m_CommandList5 )
    {
        m_CommandList5->Release();
    }
...
void CommandContext::Initialize(void)
{
    ...
    if ( true == g_bVRS1Supported )
    {
        if ( FAILED( m_CommandList->QueryInterface( __uuidof( ID3D12GraphicsCommandList5 ), (void**)&m_CommandList5 ) ) )
        {
            m_CommandList5 = nullptr;
        }
    }
...
void GraphicsContext::SetShaderingRate( D3D12_SHADING_RATE inShadingRate )
{
    if ( nullptr != m_CommandList5 )
    {
        if ( false == Graphics::g_bVRS1AdditionalRatesSupported )
        {
            inShadingRate = std::min( inShadingRate, D3D12_SHADING_RATE_2X2 );
        }

        m_CommandList5->RSSetShadingRate( inShadingRate, nullptr );
    }
}
