...
private:
    ...
    void RenderObjects( GraphicsContext& Context, const Matrix4& ViewProjMat, eObjectFilter Filter, bool inIsColourPass );
    ...
    std::vector<D3D12_SHADING_RATE> m_pMaterialShadingRate;
...
void ModelViewer::Startup( void )
{
    ...
    m_pMaterialShadingRate.resize( m_Model.m_Header.materialCount );
    for (uint32_t i = 0; i < m_Model.m_Header.materialCount; ++i)
    {
        ...
        if ( ( ::strstr( mat.name, "fabric" ) ) || ( ::strstr( mat.name, "bricks" ) ) )
        {
            m_pMaterialShadingRate[ i ] = D3D12_SHADING_RATE_2X2;
        }

        else if ( ::strstr( mat.name, "column" ) )
        {
            m_pMaterialShadingRate[ i ] = D3D12_SHADING_RATE_1X2;
        }

        else if ( ::strstr( mat.name, "arch" ) )
        {
            m_pMaterialShadingRate[ i ] = D3D12_SHADING_RATE_2X1;
        }

        else if ( ( ::strstr( mat.name, "floor" ) ) || ( ::strstr( mat.name, "ceiling" ) ) )
        {
            m_pMaterialShadingRate[ i ] = D3D12_SHADING_RATE_4X4;
        }

        else
        {
            m_pMaterialShadingRate[ i ] = D3D12_SHADING_RATE_1X1;
        }
    }
    ...
    TemporalEffects::TAAEnabledStatus = 2;
    FXAA::Enable = false;
...
void ModelViewer::RenderObjects( GraphicsContext& gfxContext, const Matrix4& ViewProjMat, eObjectFilter Filter, bool inIsColourPass )
{
    struct VSConstants
    {
    ...
    for (uint32_t meshIndex = 0; meshIndex < m_Model.m_Header.meshCount; meshIndex++)
    {
        ...
        if (mesh.materialIndex != materialIdx)
        {
            ...
            if ( true == inIsColourPass )
            {
                gfxContext.SetShaderingRate( m_pMaterialShadingRate[ mesh.materialIndex ] );
            }
        }
...
void ModelViewer::RenderLightShadows(GraphicsContext& gfxContext)
{
    ...
    m_LightShadowTempBuffer.BeginRendering(gfxContext);
    {
        ...
        RenderObjects( gfxContext, m_LightShadowMatrix[ LightIndex ], kOpaque, false );
        gfxContext.SetPipelineState(m_CutoutShadowPSO);
        RenderObjects( gfxContext, m_LightShadowMatrix[ LightIndex ], kCutout, false );
    }
...
void ModelViewer::RenderScene( void )
{
...

    {
...

        {
            ...
            RenderObjects( gfxContext, m_ViewProjMatrix, kOpaque, false );
        }

        {
            ScopedTimer _prof2(L"Cutout", gfxContext);
            gfxContext.SetPipelineState(m_CutoutDepthPSO);
            RenderObjects( gfxContext, m_ViewProjMatrix, kCutout, false );
        }
    }
    ...
    if (!SSAO::DebugDraw)
    {
...

        {
            ...
            gfxContext.SetPipelineState(m_ShadowPSO);
            RenderObjects( gfxContext, m_SunShadow.GetViewProjMatrix(), kOpaque, false );
            gfxContext.SetPipelineState(m_CutoutShadowPSO);
            RenderObjects( gfxContext, m_SunShadow.GetViewProjMatrix(), kCutout, false );
...

        {
            ...
            RenderObjects( gfxContext, m_ViewProjMatrix, kOpaque, true );

            if (!ShowWaveTileCounts)
            {
                gfxContext.SetPipelineState(m_CutoutModelPSO);
                RenderObjects( gfxContext, m_ViewProjMatrix, kCutout, true );
            }
        }
    ...
    TemporalEffects::ResolveImage( gfxContext, m_ViewProjMatrix, m_Camera.GetPreviousViewProjMatrix() );
